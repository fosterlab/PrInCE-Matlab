...
if user.skipgaussbuild==1
    disp('* NB: Skip GaussBuild? set to true in experimental_design.rtf. Skipping GaussBuild...')
    skipflag = 1;
end
...
if ~skipflag
    ...
    for ci = 1:Nchannels % loop over channels
        ...
        parfor ri = 1:Nproteins % loop over proteins
            ...
            % smooth, but only after chromatogram passes filtering
            % 5. Smooth with a boxcar filter
            clean_chromatogram = smooth(clean_chromatogram,4);
