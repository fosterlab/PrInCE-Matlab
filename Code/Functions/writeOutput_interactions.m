...
%   'Assessed with global threshol',final_Recall(1), final_Precision(1), final_TP(1), final_FP(1), ...
...
precisions = [.5 .6 .7 .8 .9];
fn=strcat(datadir,'Summary_Precision_Recall.csv');
fid= fopen(fn,'wt'); % create the summary file of the interaction output
fprintf (fid,'%s,%s,%s,%s,%s,%s\n',... %header for OutputGaus output
  'Precision TP/(TP+FP)', 'Recall TP/(TP+FN)', 'Number of TP interactions','Number of FP interactions',...
  'Number of novel interactions','Number of possible gold standard interactions (TP+FN)');
for ii = 1:length(precisions)
  I = interaction_final.precisionDropoutavg>=precisions(ii);
  ntp = sum(interaction_final.proteinInCorum(I)==1 & interaction_final.interactionInCorum(I)==1);
  nfp = sum(interaction_final.proteinInCorum(I)==1 & interaction_final.interactionInCorum(I)==0);
  nnov = sum(interaction_final.proteinInCorum(I)==0 & interaction_final.interactionInCorum(I)==0);
  fprintf(fid,'%6.4f,%6.4f,%6.4f,%6.4f,%6.4f,%6.4f\n',...
    ntp/(ntp+nfp),ntp/(all_positives),ntp,nfp,nnov,all_positives);
end
%fprintf (fid,'%s,%6.4f,%6.4f,%6.4f,%6.4f,%6.4f,,,%6.4f,\n',...
